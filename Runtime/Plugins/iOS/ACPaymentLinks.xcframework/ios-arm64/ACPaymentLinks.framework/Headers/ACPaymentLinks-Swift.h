...
/// Sets the browser mode used by the checkout flow.
/// This API provides a single, strongly-typed entry point that is callable from Swift,
/// Objective-C, and Objective-C++ (.mm). Internally, it updates the controller’s
/// <code>useExternalBrowser</code> flag:
/// <ul>
///   <li>
///     <code>.external</code>  → <code>useExternalBrowser = true</code>
///   </li>
///   <li>
///     <code>.internal</code>  → <code>useExternalBrowser = false</code>
///   </li>
/// </ul>
/// If the SDK has not been initialized yet (i.e., the internal controller is <code>nil</code>),
/// the function returns <code>nil</code> and no changes are applied.
/// \param mode The desired browser mode (<code>.external</code> or <code>.internal</code>).
///
///
/// returns:
/// The applied mode string (“external”/“internal”), or <code>nil</code> if the controller isn’t initialized.
+ (NSString * _Nullable)setBrowserMode:(enum BrowserMode)mode;
...
/// \param customerId A unique identifier representing the customer for whom the checkout flow is being initialized.
///
/// \param platformIntegrationVersion (Optional) A string that identifies the integrating platform/bridge version
/// (e.g., Unity/ReactNative wrapper version). This value may be sent with requests for diagnostics,
/// compatibility, and analytics. If not provided, an empty string is used.
///
+ (void)initializeWithConfigModel:(ACConfigModel * _Nonnull)configModel customerId:(NSString * _Nonnull)customerId platformIntegrationVersion:(NSString * _Nullable)platformIntegrationVersion;
...
@interface CancelOrderResponse : NSObject
@property (nonatomic) BOOL isOrderCancelled;
@end
